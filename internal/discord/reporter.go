...
func SendKillReport(s *discordgo.Session, channelID string, report models.BossKillReport) {
	...
	embed := &discordgo.MessageEmbed{
		Author: &discordgo.MessageEmbedAuthor{
			...
			IconURL: fmt.Sprintf("https://sirus.su/api/base/%s/guild/%d/emblem.webp", report.Realm, report.GuildID),
			URL:     fmt.Sprintf("https://sirus.su/base/guilds/%s/%d", report.Realm, report.GuildID),
		},
...
func buildFields(report models.BossKillReport, ddBlocks, healBlocks []string, lootsBlock []models.LootReport) []*discordgo.MessageEmbedField {
	...
	if len(lootsBlock) > 0 {
		...
		for _, item := range lootsBlock {
			if item.Name != "" {
				fmt.Fprintf(&sb, "x%d [%s](https://sirus.su/base/item/%d/%s)\n",
					item.Count, item.Name, item.ID, report.Realm)
			}
		}
		if sb.Len() > 0 {
			fields = append(fields, &discordgo.MessageEmbedField{
...
func generateJoke() string {
	jokes := []string{
		...
		"LazyCatBot — Логи обработаны. Не пингуйте меня больше сегодня. 💤",
		"LazyCatBot — Мяу. Лут забрали, босс упал, я пошел. 🐈",
		...
		"LazyCatBot — Этот босс пал, но ваш ДПС все еще вызывает вопросы. ❓",
		"LazyCatBot — Заливаю логи, пока вы делите шмотки. 💍",
		"LazyCatBot — РТ окончено? Слава богу, а то клавиатура греться перестала. ⌨️",
		"LazyCatBot — Ваша сыгранность потрясает... в худшем смысле. 😒",
		"LazyCatBot — Босс упал, но моральный ущерб мне никто не возместит. 🐈‍⬛",
		"LazyCatBot — Мяу-уведомление: босс мертв, идите есть. 🐟",
		"LazyCatBot — Зафиксировал ваш «успех». Натянули с трудом, но ладно. 🐾",
		"LazyCatBot — Ваша ротация звучит как тыгыдык по клавиатуре. 🎹",
		"LazyCatBot — Логи на сервере. Можете продолжать спорить из-за анролла. 💎",
		"LazyCatBot — Наконец-то вы закончили. Я уже устал смотреть на эти вайпы. 😒",
		"LazyCatBot — Босс лежал, рейд стоял, кот спал. Все стабильно. 🐈",
		"LazyCatBot — Записал килл в историю. Теперь не мешайте мне умываться. 🐾",
		"LazyCatBot — Уровень вашей игры: «Кот прошлся по мышке». 🐭",
		"LazyCatBot — Опять дотянули до берсерка? Ну вы и ленивые... Уважаю. 😼",
		"LazyCatBot — Логи в базе. Пойду сброшу что-нибудь со стола от радости. 📦",
		"LazyCatBot — Вайпы закончились? Наконец-то, я уже заждался. 🥱",
		"LazyCatBot — Этот лог вызвал у меня желание свернуться клубочком и всё забыть. 💤",
		"LazyCatBot — Нажмите Alt+F4 и дайте коту поспать в тишине. 💻",
		"LazyCatBot — Рейд пройден, но гордиться тут особо нечем. 😒",
	}
